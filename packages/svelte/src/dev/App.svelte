<script lang="ts" >
	...
	let items = $state(loadSlides());

	async function loadSlides() {
		return await getRandomSlides(limit);
	};
</script>
...
<header class="header">
	...
	<fieldset class="nav-controls">
		<button onclick={() => items = loadSlides()} title="Shuffle slides" aria-label="Shuffle slides">
		...
		<button onclick={() => darkTheme.switch()} class:active={$darkTheme} title="Switch theme" aria-label="Switch theme">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
				<path
		...
		<button onclick={() => (controlPanel = !controlPanel)} title="Open sidebar" aria-label="Open sidebar">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
				<path
