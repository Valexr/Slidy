(() => {
  ...
  function slidy(node, options = {
    ...
    function move(pos, transition = 0) {
      ...
      const translate = (vertical) => vertical ? `0, ${-position2}px, 0` : `${-position2}px, 0, 0`;
      const styles = {
    ...
    function to(index, target = null) {
      ...
      const child2 = find.child(node, index);
      const ix = options.loop ? find.index(node, position2, child2, options.vertical, options.align) : index;
      ...
      move(pos - position2, options.duration);
    }
    function track(timestamp) {
      RAF(function track2(time) {
        const v = 1e3 * (position2 - frame) / (1 + (time - timestamp));
        velocity = (2 - options.gravity) * v + maxMin(1, 0, 1 - options.gravity) * velocity;
        timestamp = time;
        frame = position2;
        rak = RAF(track2);
      });
    ...
    function onMove(e) {
      const delta = (reference - coordinate(e, options.vertical)) * (2 - options.gravity);
      reference = coordinate(e, options.vertical);
      move(delta);
    }
    function onUp(e) {
