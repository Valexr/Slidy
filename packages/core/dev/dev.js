var Slidy = (() => {
  ...
  function slidy(node, {
    ...
    const listen = (node2, events, off = false) => events.forEach(([event, handle]) => off ? node2.removeEventListener(event, handle, true) : node2.addEventListener(event, handle, true));
    const windowEvents = [
      ["touchmove", onMove],
      ["mousemove", onMove],
      ["touchend", onUp],
