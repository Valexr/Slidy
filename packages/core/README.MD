...
<!-- | `layout`     | `reel`             | `string`   | Animation layout: `'deck', 'grid', 'reel', 'stack'` | -->
...
| `deck`       | `false`            | `boolean`  | Animation deck layout |
...
type AnimationArgs = {
    node: HTMLElement; // slidy(node)
    child: Child; // extended childNode object
    options: Partial<Options> // slidy(options)
