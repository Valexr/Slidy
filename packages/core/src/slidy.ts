...
export function slidy(node: HTMLElement, options: Options = {}): {
    ...
    onMounted(node)
        .then((childs: HTMLCollection) => {
            ...
            if (PARENT) {
                PARENT.onresize = () => to(index) // add resizeObserver
    ...
    function to(index: number, target: number | null = null) {
        ...
        let pos = target
            ? snap
                ...
                : target
            : target === 0 ? 0 : find.position(node, ix, axis, align, loop)
    ...
    function scroll({ target, amplitude, duration, timestamp }) {
        if (amplitude) {
            RAF(function scroll(time: number) {
                ...
                raf = Math.abs(delta) > 0.5 ? RAF(scroll) : null
    ...
    function onWheel(e: WheelEvent) {
        ...
        // console.log(axisCoord(e, axis))
        if (e.shiftKey) to(index - Math.sign(e.deltaY))
        else if (snap || clamp) wheeltime = setTimeout(() => (to(index), wheeling = false), 100)
    }
