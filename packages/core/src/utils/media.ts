export const media = mediaStorage({ key: 'slidy-media' })

function mediaStorage(storage?: Partial<BrowserStorage>, queries?: Partial<MediaQuery>): MediaQuery {
    const base = {
        queries: {
            xs: '(max-width: 480px)',
            sm: '(max-width: 600px)',
        ...
        storage: { type: 'session', key: 'media-query' }
    }
