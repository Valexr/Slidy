...
Persist media-query matches object in any browser storage. Storage key `mediaStorage`.

```ts
const storage: Storage = undefined | sessionStorage | localStorage
```
...
<head>
    <script type="module">
        ...
        const media = mediaStorage({ storage: sessionStorage })
    </script>
</head>
```
