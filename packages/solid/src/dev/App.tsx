...
const App: Component = () => {
    const [animation, setAnimation] = createSignal(() => translate);
    const [axis, setAxis] = createSignal<'x' | 'y'>('y');
    const [easing, setEasing] = createSignal(() => linear);
    const [groups, setGroups] = createSignal(0);

    const [vertical, setVertical] = createSignal(true);
    const [clamp, setClamp] = createSignal(0);
    const [duration, setDuration] = createSignal(450);
    const [gravity, setGravity] = createSignal(1.45);
    const [width, setWidth] = createSignal('auto');
    const [snap, setSnap] = createSignal<'start' | 'center' | 'end' | undefined>('center');
    const [loop, setLoop] = createSignal(false);
    const [gap, setGap] = createSignal(15);

    const [index, setIndex] = createSignal(0);

    const [controlPanelVisible, setControlPanelVisible] = createSignal(false);

    const [slides, setSlides] = createSignal<Slide[]>([]);

    const loadSlides = () => {
        const local = localStorage.getItem('slides');
        ...
        getRandomSlides().then((s) => {
            if (s?.length) {
                localStorage.setItem('slides', JSON.stringify(s));
                setSlides(s);
        ...
        <>
            <header class="header">
                ...
                <fieldset class="nav-controls">
                    ...
                    <button onClick={() => setControlPanelVisible((v) => !v)} title="Open sidebar">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
            ...
            <Sidemenu controlPanelVisible={controlPanelVisible()} setControlPanelVisible={setControlPanelVisible}>
                <ControlPanel
                    vertical={vertical}
