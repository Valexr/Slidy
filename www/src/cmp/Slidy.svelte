    ...
    >
        {#if slides}
            {#each slides as item, i (item.id)}
                <li
                    bind:this={nodes[i]}
                    use:action.intersection={{ intinit, ...intopts }}
                    on:intersection|stopPropagation={(e) =>
                        onIntersection(e, item.ix)}
                    class={slide.class}
...
<script>
    ...
    $: if (slides.length)
        el = {
            active: slides[
                options.loop ? Math.floor(slides.length / 2) : index
            ],
    ...
    $: if (nodes.length) {
        firstsize = options.axis === 'y' ? el.firstheight : el.firstwidth;
        lastsize = options.axis === 'y' ? el.lastheight : el.lastwidth;
        activesize = options.axis === 'y' ? el.activeheight : el.activewidth;
        beforesize = options.axis === 'y' ? el.beforeheight : el.beforewidth;
        aftersize = options.axis === 'y' ? el.afterheight : el.afterwidth;
        fullsize = options.axis === 'y' ? el.fullheight : el.fullwidth;
    ...
    function slidyLoop() {
        ...
        if (options.loop) {
            index = i = el.active.ix;
        } else {
            pos >= beforesize || pos <= -aftersize
                ? (pos = pos / 1.5)
                : (pos = pos);
        }
    }
    ...
    function slidyStop() {
        ...
        const nulled = (direct) => {
            if (direct) {
                if (options.loop) {
                    ...
                    tick().then(() => (index = i = el.active.ix));
                } else {
                    index = direct;
                    pos = px = htx = 0;
                }
        ...
        if (pos > lastsize / 2 || speed > 0.25) {
            ...
            if (options.loop) {
                ...
                // pos += lastsize - pos + (dontdrag === true ? px * speed : 0)
                nulled(
                    (i = index -= dontdrag === true ? Math.round(1 * speed) : 1)
                );
                // setTimeout(() => {
